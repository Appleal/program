<template>
  <c-table-scroll :data="tableData">
    <list-table title="订单列表" order='true'>
      ...
      <slot slot="query">
        <div class="custom-query-item">
          <div class="tabNav">
            <ul class="tabChange">
              <li class="active"> <router-link to="/order" class="allAcitve">订单列表</router-link>  </li>
        ...
        <div class="custom-query-item">
          ...
          <el-radio-group v-model="state" size="mini" style="margin-right: 41px;" @change="query()">
            <el-radio-button label="">不限</el-radio-button>
            <el-radio-button label="100">待支付</el-radio-button>
            <el-radio-button label="200">待发货</el-radio-button>
            <el-radio-button label="300">已发货</el-radio-button>
            <!--<el-radio-button label="订单退回">订单退回</el-radio-button>-->
            <el-radio-button label="400">已收货</el-radio-button>
        ...
        <div class="custom-query-item">
          ...
          <el-radio-group v-model="orderFrom" size="mini" style="margin-right: 41px;" @change="query()">
            ...
            <el-radio-button :label="0">标品订单</el-radio-button>
          </el-radio-group>
        </div>
        <div>
          <span>精准查询：</span>
      ...
      <div class="custom-table">
        <el-table style="width: 100%"
          ...
          <el-table-column label="订单金额" width="90px" align="center" prop="payableAmount">
            <template slot-scope="scope">
              <span>￥{{ tableData[scope.$index].payableAmount }} </span>
            </template>
          </el-table-column>
          <el-table-column label="已支付" width="90px" align="center" prop="paid">
            <template slot-scope="scope">
              <span>￥{{ tableData[scope.$index].paid }} </span>
            </template>
          </el-table-column>
          <el-table-column label="订单状态" width="80" align="center" prop="stateName">
            <template slot-scope="scope">
...
export default {
  ...
  methods: {
    ...
    rowAction (index, bottoms, rowIndex, row) {
      ...
      this.$router.push({path: '/order/info', query: {tid: rowId, state}})
    },
    query () {
